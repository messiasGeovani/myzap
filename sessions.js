...
module.exports = class Sessions {
  ...
  static async initSession(sessionName) {
    ...
    if (Sessions.options.jsonbinio_secret_key !== undefined) {
      ...
      if (response.data.WAToken1 !== undefined) {
        ...
        console.log(
          "puxou isso: " + JSON.stringify(session.browserSessionToken)
        );
      } else {
    ...
    if (process.env.ENGINE === "VENOM") {
      const client = await venom.create(
        ...
        (statusSession, session) => {
          console.log(
            "#### status=" + statusSession + " sessionName=" + session
          );
        },
        {
      ...
      console.log(
        "usou isso no create: " + JSON.stringify(browserSessionToken)
      );
  ...
  static async setup(sessionName) {
    ...
    await session.client.then((client) => {
      client.onStateChange((state) => {
        ...
        if (state == "CONNECTED") {
          if (
            Sessions.options.jsonbinio_secret_key !== undefined &&
            session.browserSessionToken == undefined
          ) {
            //se informou secret key pra salvar na nuvem
            setTimeout(async () => {
              ...
              var config = {
                ...
                url:
                  ...
                  Sessions.options.jsonbinio_bin_id,
                headers: {
  ...
  static async sendText(req, res) {
    ...
    if (session) {
      if (session.state == "CONNECTED") {
        ...
        return res
          .status(401)
          .json({ result: "error", message: session.state });
      }
    } else {
  ...
  static async getAllChatsNewMsg(sessionName) {
    ...
    if (session) {
      if (session.state == "CONNECTED") {
        var resultGetAllChatsNewMsg = await session.client.then(
          async (client) => {
